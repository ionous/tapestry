...
func init() {
	Zt_Directive = typeinfo.Flow{
		...
		Markup: map[string]any{
			"comment":  []string{"Starts a parser scanner.", "This is generated by story statement \"DefineNamedGrammar\"", "so that grammar doesn't have to import weave."},
			"internal": true,
		},
	}
	Zt_Action = typeinfo.Flow{
		...
		Terms: []typeinfo.Term{{
			Name: "action",
			Markup: map[string]any{
				"comment": []string{"Name of the action.", "See also: [DefineAction]."},
			},
			Type: &prim.Zt_Text,
		}, {
			Name:     "arguments",
		...
		Markup: map[string]any{
			"comment": []string{"Run a script defined action.", "The action will be parameterized with the actor performing the action,", "and up to two other nouns. The actor is always the player's,", "the nouns are whichever were matched by [Noun].", "This command usually appears last in a series of sub-scanners,", "after all of the earlier scanners have successfully matched."},
		},
	}
	...
	Zt_ChooseOne = typeinfo.Flow{
		...
		Markup: map[string]any{
			"comment": []string{"Attempts to match exactly one of its sub-scanners.", "Its sub-scanners are evaluated in the order listed;", "stopping after the first successful match has occurred."},
		},
	}
	Zt_Noun = typeinfo.Flow{
		...
		Terms: []typeinfo.Term{{
			...
			Label: "noun",
			Markup: map[string]any{
				"comment": []string{"The kind of the noun that should be allowed to match.", "Most often this is the most generic kind: \"objects\".", "That allows the action to respond to the player", "when an unexpected or undesired noun was entered."},
			},
			Type: &prim.Zt_Text,
		}},
		Slots: []*typeinfo.Slot{
		...
		Markup: map[string]any{
			"comment": []string{"Attempt to find a noun matching one or more words", "entered by the player. Out of all the nouns in a game world", "the specific nouns matched will, by default, depend on what", "objects are visible to the player."},
		},
	}
	Zt_Refine = typeinfo.Flow{
		...
		Markup: map[string]any{
			"comment": []string{"Uses the last noun in a series of sub-scanners as a source", "for finding for the first noun in the series.", "For example, in the phrase \"take book from table\"", "if there was one book on a table, and many books on a bookshelf,", "refine would ensure that the book on the table was selected."},
		},
	}
	Zt_Reverse = typeinfo.Flow{
		...
		Markup: map[string]any{
			"comment": []string{"Assuming that the first and last sub-scanners in a set", "match nouns, reverse the order of those nouns when sending", "them to an action."},
		},
	}
	...
	Zt_Words = typeinfo.Flow{
		...
		Terms: []typeinfo.Term{{
			...
			Repeats: true,
			Markup: map[string]any{
				"comment": []string{"One or more synonyms to match.", "Only the first word matched will be used.", "For example: \"pluck\", \"pick\", \"strum\"", "would match either \"pluck the guitar\",", "or \"strum the guitar\"."},
			},
			Type: &prim.Zt_Text,
		}},
		Slots: []*typeinfo.Slot{
		...
		Markup: map[string]any{
			"comment": []string{"Match text exactly as typed by the player.", "To match two words that must appear together", "use a [Sequence] containing two separate word commands.", "For example, to match the phrase \"take off\", use:", "  Sequence:", "    - One word: \"take\"", "    - One word: \"off\""},
		},
	}
}
