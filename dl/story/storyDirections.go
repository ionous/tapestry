...
func (op *MapDeparting) Weave(cat *weave.Catalog) error {
	return cat.Schedule(assert.RequireDeterminers, func(w *weave.Weaver) (err error) {
		...
		} else if e := assert.AssertNounValue(cat, Tx(otherRoom.uniform, "rooms"), door.name, "destination"); e != nil {
...
func mapDirect(cat *weave.Catalog, room, otherRoom, exitDoor helperNoun, mapDir MapDirection) (err error) {
	if dir := lang.Underscore(mapDir.Str); len(dir) == 0 {
		...
		if e := cat.AssertNounKind(exitDoor.name, "doors"); e != nil {
			...
			err = e // ^ put the exit in the current room
		} else if e := assert.AssertNounValue(cat, Tx(exitName, "door"), room.name, "compass", dir); e != nil {
			err = e // ^ set the room's compass to the exit
		} else if e := assert.AssertNounValue(cat, Tx(otherName, "rooms"), exitDoor.name, "destination"); e != nil {
			...
			if generateExit {
				...
				if e := assert.AssertNounValue(cat, B(true), exitName, "scenery"); e != nil {
					err = e
				} else if e := assert.AssertNounValue(cat, B(true), exitName, "privately_named"); e != nil {
					err = e
				}
			}
		}
