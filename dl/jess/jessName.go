...
func (op *Name) BuildNoun(q Query, w weaver.Weaves, props NounProperties) (ret DesiredNoun, err error) {
	if an, created, e := ensureNoun(q, w, op.Matched, &props); e != nil {
		err = e
	} else if e := writeKinds(w, an.Name, props.Kinds); e != nil {
		err = e
	} else {
		n := DesiredNoun{Noun: an.Name, Traits: props.Traits, CreatedKind: an.Kind}
		if created {
