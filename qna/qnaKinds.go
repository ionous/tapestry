...
type fieldSet struct {
	...
	init   []core.Assignment // fix? move this into g.Kinds ( probably as a callback Init()g.Value to avoid dependency on package rt )
}
	...
	for next, i := kind, len(path); ; {
		if fs, e := run.getFields(next); e != nil {
			...
			if cnt := len(fs.init); cnt > 0 {
				if out.init == nil {
					out.init = make([]core.Assignment, len(out.fields))
				}
...
func (run *Runner) getFields(kind string) (ret fieldSet, err error) {
	if fs, e := run.qdb.FieldsOf(kind); e != nil {
		...
		var init []core.Assignment // init is so often empty, only allocate on demand.
		for i, f := range fs {
			...
			if prog := f.Init; len(prog) > 0 {
				if val, e := decodeAssignment(f.Affinity, prog, run.signatures); e != nil {
					...
					if init == nil {
						init = make([]core.Assignment, len(fs))
					}
					init[i] = val
				}
			}
