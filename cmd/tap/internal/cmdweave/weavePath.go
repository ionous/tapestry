	...
	if outFile, e := filepath.Abs(outFile); e != nil {
		...
		if db, e := tables.CreateBuildTime(outFile); e != nil {
			...
			if q, e := qdb.NewQueries(db, query.NewDecoder(story.AllSignatures)); e != nil {
				...
				if pen, e := cat.SceneBegin("tapestry", pos, nil); e != nil {
					err = e
				} else {
		...
		if scene != nil {
			...
			if scene, req, e := scene.GetSceneReqs(run); e != nil {
				...
				if scene != "tapestry" {
					...
					if pen, e := cat.SceneBegin(scene, pos, nil); e != nil {
						err = e
					} else {
			...
			if name := ent.Name(); name[0] == '_' || name[0] == '.' {
				...
				if shortName, ext := files.SplitExt(name); ext.Story() {
					...
					if fp, e := fsys.Open(fullpath); e != nil {
						...
						if src, e := flex.ReadStorySource(namedPath, bufio.NewReader(fp)); e != nil {
							err = fmt.Errorf("couldn't read %q %s", namedPath, e)
						} else {
							//
							src := wrapScene(scene, shortName, namedPath, src)
							if e := story.Weave(cat, src); e != nil {
								err = fmt.Errorf("couldn't import %q because %s", namedPath, e)
							}
						}
						// errors above rely on *not* exiting the loop, so we can close.
						fp.Close()
					}
				}
			}
