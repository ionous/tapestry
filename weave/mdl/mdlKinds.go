...
type kindInfo struct {
	rowid        int64  // autogenerated sqlite id
	uid          string // unique id for the kind; todo: replace rowid with this.
...
func (ki *kindInfo) class() classInfo {
	return classInfo{id: ki.rowid, name: ki.name, fullpath: ki.fullpath()}
}
...
func (ki *kindInfo) fullpath() string {
	if ki.rowid > 0 && len(ki._fullpath) == 0 {
		ki._fullpath = "," + strconv.FormatInt(ki.rowid, 10) + ki.path
	}
	return ki._fullpath
}
...
func (pen *Pen) findKind(kind string) (ret kindInfo, err error) {
	...
	limit 1`, // order by rank means the lowest number is first
			...
			&ret.domain, &ret.rowid, &ret.name, &ret.path, &rank)
		switch e {
		case nil:
